...
**AI-powered research idea generation through adversarial multi-agent debate.** IdeaForge crawls top ML conference papers (ICLR, NeurIPS, ICML) from OpenReview, trains a calibrated AI reviewer on 50K+ real peer reviews using prompt evolution (GEPA), and runs a 3-agent adversarial debate loop (Critic-Proposer-Judge) to iteratively develop and stress-test research ideas until they meet publication-quality standards.
...
- **Calibrated AI judge** — GEPA-optimized prompt achieves 91.7% accuracy against real reviewer scores (within ±1.5)
- **OpenReview integration** — crawls papers and reviews from ICLR, NeurIPS, ICML via the OpenReview API
- **FAISS-based paper retrieval** — embeds 50K+ papers for similar-paper grounding during evaluation
