...
- **FAISS-based paper retrieval** — embeds crawled papers for similar-paper grounding during evaluation
- **Full reproducibility** — one-command setup, pre-trained judge included, all skill files and training artifacts in repo
...
Default mode crawls ~100 representative papers for quick setup. The included judge prompt and skills were trained on the full 50K corpus — use `--full` mode or `python judge_training/optimize.py` to retrain from scratch.
